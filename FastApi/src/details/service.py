...
import pandas as pd
from src.details.schemas import QueryResponseBoxOffice, QueryResponseFilmInfo, ActorInfo, QueryResponseActorInfo
from src.details.utils import box_office_validate, film_validate
# from schemas import QueryResponseBoxOffice, QueryResponseFilmInfo, ActorInfo, QueryResponseActorInfo, DetailsDataInItem
# from utils import box_office_validate, film_validate
...
FILM_COLS = ["Movie_name", "Movie_release_date_year", "Movie_runtime", "Movie_box_office_revenue"]
# ACTOR_COLS = ["Actor_name", "Actor_gender", "Actor_height", "Actor_date_of_birth"]
...
def get_top_n_box_office_films(n):
    ...
    '''
    df = pd.read_csv(MOVIES_PATH).sort_values(by=['Movie box office revenue'], ascending=False)
    df.columns = df.columns.str.replace(' ', '_')
    final_lst = []
    for i in range(n):
        try:
            cur_dct = {BOX_OFFICE_COLS[j]: df[BOX_OFFICE_COLS].values[i][j] for j in range(len(BOX_OFFICE_COLS))}
...
def get_film_info(input_obj):
    ...
    for i in range(info.shape[0]):
        try:
            cur_dct = {FILM_COLS[j]: info[FILM_COLS].values[i][j] for j in range(len(FILM_COLS))}
...
def get_actor_info(input_obj):
    ...
    info["Actor gender"] = info["Actor gender"].apply(lambda x: 'Male' if x == 'M' else 'Female')
    info_dict = {}
    try:
...
# print(get_top_n_box_office_films(82))
#
# print(get_film_info(DetailsDataInItem(query='King Kong')))
#
# print(get_actor_info(DetailsDataInItem(query='Brad Pitt')))