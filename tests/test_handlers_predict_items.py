...
sys.path.append("..")
from Telegram_bot.handlers.predict_items import (  # noqa: E402
    cmd_more,
    query_chosen_more,
    query_chosen_incorrectly_more,
    n_predictions_chosen_incorrectly_more,
    n_predictions_chosen_more,
)
...
mocked_reply = (
    "<u> For your query 'magic' we can recommend next movies: </u>\n\n"
    ...
    "<b> Synopsis: </b> Magic Journeys looked at the world through the eyes of a child. "
    "The film started with children running through a meadow and looking at clouds. "
    "Someone blew on a dandelion and the seeds then flew away, turning into stars and "
    "then turned into the sun. Next the kids were seen flying a kite at the beach. The "
    "kite then turned into a bird, a fish, a school of fish, a flock of birds, bird wings, "
    "a Pegasus, a horse and then finally into a merry-go-round. While the children rode "
    "the carousel, they began\n\n"
    ...
    "<b> Synopsis: </b> Set in a dystopian future, 2017, ex-army officer John Henry "
    "Brennick  and his wife Karen  are attempting to cross the US-Canada border to "
    "Vancouver to have a second child. Strict one-child policies forbid a second "
    "pregnancy, but the couple believe they are justified because their first child "
    "died at birth. Brennick is caught  and sentenced to 31 years in a private "
    "maximum security prison run by the \"MenTel Corporation\". To maintain discipline "
    "all inmates are implanted with \"Intestinators\" which\n\n"
)
...
async def test_query_chosen_more():
    ...
    answer = calls.send_message.fetchone().text
    assert (
        "Спасибо. Теперь, пожалуйста, выберите количество рекомендаций (для каждого запроса):"
        in answer
    )
...
async def test_n_predictions_chosen_incorrectly_more():
    bot = MockedBot(
        MessageHandler(
            n_predictions_chosen_incorrectly_more, state=Request.n_predictions
        )
    )
    message = MESSAGE.as_object(text="invalid")
    calls = await bot.query(message)
