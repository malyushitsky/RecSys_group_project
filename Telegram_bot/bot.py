...
async def cmd_cancel(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(
        text="Отменено. Можем начать заново",
        reply_markup=ReplyKeyboardRemove()
    )
...
async def set_main_menu(bot: Bot):
    main_menu_commands = [
        ...
        BotCommand(command='/cancel',
                   description='Отмена обработки в командах, ожидающих ввод'),
        BotCommand(command='/rec',
                   description='Рекомендации по описанию'),
        BotCommand(command='/batch_rec',
                   description='Рекомендации по нескольким описаниям'),
        BotCommand(command='/topbox',
                   description='Топ фильмов по кассовым сборам'),
        BotCommand(command='/actor',
                   description='Краткая информация об актере'),
        BotCommand(command='/film',
                   description='Краткая информация о фильме'),
        BotCommand(command='/llm',
                   description='Рекомендации по описанию от LLm')
    ]
