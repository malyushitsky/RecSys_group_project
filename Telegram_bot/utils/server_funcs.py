...
def get_reply(endpoint, json=None, params=None):
    error_fl = False
    try:
        server_reply = requests.post(URL_MAIN + endpoint, params=params, json=json).json()
    ...
    if not error_fl:
        ...
        if endpoint in [BOX_OFFICE_ENDPOINT, FILM_ENDPOINT]:
            ...
            final_html += f"<u> For your query '{json.get('query', 'No data')}' we can recommend next movies: </u>\n\n"
            for query in server_reply['response']:
                final_html += process_reply(query)
            ...
            for obj in server_reply['objects']:
                final_html += (f"<u> For your query '{obj.get('query', 'No data')}' "
...
# print(process_reply({'Movie_name': 'Avatar', 'Movie_release_date_year': 2009, 'Movie_box_office_revenue': 2782275172.0} ))

# print(get_prediction(user_data))
